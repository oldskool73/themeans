 ...
 */
angular.module('tm.fire-location', ['firebase']).service('FireConfig', [
  'Firebase',
  '$window',
  '$timeout',
  '$interval',
  'Parse',
  '$rootScope',
  function FireConfig(Firebase, $window, $timeout, $interval, Parse, $rootScope) {
    ...
    $rootScope.$on('gps-position-update', function (evt, location) {
      ...
      geoFireEndpoint.set(Parse.User.current().id, [
    ...
    $interval(function () {
      geoFireEndpoint.set('xxxyyyzzz', [
        -33.8685763,
        151.2097666 + Math.random() / 100
      ]);
    }, 4000);
    (function (ref) {
      serverTimeOffset = 0;
