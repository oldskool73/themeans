...
var d=new e.Query(i),f=[a,e.User.current().get("profile").id],j=h.createHash(f.sort().join(""));return d.equalTo("profileIdsHash",j),d.find({success:function(a){return a.length?a[0]&&"pending"===a[0].get("requestStatus")?c.resolve("pending"):a[0]&&"accepted"===a[0].get("requestStatus")?c.resolve("accepted"):(k.info("Parse Error",a),void c.reject({message:"Please try again in a few moments, or contact support."})):c.resolve("none")},error:function(a){k.error("Parse Query Error: "+a.message,a.code),c.reject({message:"Please try again in a few moments, or contact support."})}}),c.promise},this.getConnectionsForDisplay=function(){return o(!1)},this.getConnectionsForEdit=function(){return o(!0)},this}],this});