  ...
  this.$get = [
    ...
    function ($q, $timeout, Settings, Parse, tmLocalStorage, $log) {
      ...
      function getUserRoles(user) {
        var deferred = $q.defer(), queryRoles = new Parse.Query(Parse.Role), cacheKey = options.rolesCacheKey, roles;
        user.fetch().then(function () {
          queryRoles.equalTo('users', user);
          queryRoles.find().then(function (parseRoles) {
      ...
      function getUserRolesSecured(user) {
        var deferred = $q.defer(), cacheKey = options.rolesCacheKey;
